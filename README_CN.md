...
- **服务端派生的调用者身份**：敏感工具（`save_memory` / `save_skill` / `create_job` / `list_jobs` / `update_job` / `delete_job` / `what_do_you_know` / `forget_memory` / `create_github_issue` / `create_issue_proposal` / `list_issue_proposals` / `reject_issue_proposal` / `create_issue_from_proposal` / `create_low_risk_pr_from_proposal` / `run_local_cli_tool`）从飞书事件流派生调用者身份，不信任工具参数——社工提示无法假冒他人操作
...
- **追加式审计日志（v0.11.0+）**：`~/.codex/channels/lark/audit.log` 以紧凑文本行记录每次敏感工具调用（时间 / log id / `audit` / 工具名 / 结果 / 调用者 / 脱敏参数），运营者可事后回溯查看本机上发生了什么
...
| `LARK_CODEX_EXEC_TRACE_LOG` | `~/.codex/channels/lark/trace.log` | 本地 codex exec tool trace 文本日志路径 |
...
progress 消息。`compact` 记录工具名、参数摘要、状态、耗时和错误摘要；
`full` 记录脱敏/截断后的事件结构。trace 行靠前位置会带 log id：普通飞书消息
turn 使用 source message id，cronjob prompt turn 使用 cronjob name。audit 和 trace
日志共享[本地诊断日志文本格式](docs/local-diagnostic-logs.md)。
...
exec delivery 也内置父进程 action bridge，用于处理子 `codex exec` 进程不能安全调用当前 MCP server 的内置动作：`save_memory`、`create_job`、`list_jobs`、`update_job`、`disable_job`、`delete_job`、`upsert_job`、`create_default_review_jobs`、`create_github_issue`、`create_issue_proposal`、`list_issue_proposals`、`reject_issue_proposal`、`create_issue_from_proposal`、`create_low_risk_pr_from_proposal`、`run_local_cli_tool`、`send_message` 和 `recall_message`。父进程会为每个 `codex exec` turn 创建 owner-only 的 action JSONL 侧通道，只把文件路径和 per-turn token 交给子进程；子进程把结构化 action request 写入该侧通道，stdout 始终只作为用户可见回复文本。父进程校验 token/schema，拒绝子进程提供的身份字段，使用当前飞书事件派生调用者身份，在本地执行通过校验的动作；格式错误的 action request 会被标记为失败，而不是递归加载 Lark MCP server。`send_message` 可用插件运行时身份发送本地 image/file、当前入站消息已下载的第一张图片，或 quoted/replied message 的第一张图片；也支持有序 `kind=rich` 文本+图片 parts，优先发送一条飞书富文本 post，失败时按顺序拆分消息并保持 thread context。音视频和交互卡片仍是后续独立设计切片。

当用户明确授权创建 GitHub issue，且 repo/title/body 都已明确时，`create_github_issue`
会通过内置 GitHub HTTP path 或可选的已配置本地 CLI override 直接创建 issue。周期复盘流程应先创建 issue proposal，而不是直接写 GitHub：`create_issue_proposal`
会在本地 Lark channel home 下保存待审批 proposal，飞书报告中询问 maintainer 是否提 issue；
只有用户明确授权后，`create_issue_from_proposal` 才能走插件内置的 proposal filing
path；该路径使用配置好的 GitHub token 和 HTTP API。高级用法仍可显式传入 `tool` 使用 `external_issue_create` 这类本地 CLI
override；`gh` 这类原始可执行文件名不会被当成 tool override。已经创建 GitHub issue、且标记为
`low-risk-auto-pr-eligible` 的 proposal，可继续通过 `create_low_risk_pr_from_proposal`
调用 `gh_low_risk_pr_create` 这类受控 wrapper 开 PR，用于执行低风险范围、验证和禁止自动 merge/release 等本地策略。

`create_default_review_jobs` 会创建两个内置 prompt cronjob：`plugin-self-review` 和
`plugin-low-risk-auto-fix`。两者创建后都是 `status=paused`；
`plugin-self-review` 默认 `weekly on fri at 17:00`，
`plugin-low-risk-auto-fix` 默认 `weekly on fri at 18:00`。它们不会自动运行；用户必须显式 resume
后才会开始自复盘或低风险自修复。自复盘任务只创建 proposal 并汇报到飞书；低风险自修复任务只允许
docs/tests/metadata 等低风险工作，不能自动 merge PR，也不能自动发版。
...
`create_github_issue` 和 `create_issue_from_proposal` 默认使用内置 GitHub HTTP filing path；
该路径需要配置 `LARK_GITHUB_TOKEN`、`GH_TOKEN` 或 `GITHUB_TOKEN`。显式传入 `tool`
时仍可使用高级本地 CLI override；
该值必须是 `local-cli-tools.json` 里配置的工具名，不是原始可执行文件名。此时需要在
对应工具的 `paramAllowlist` 中包含 `--repo`、`--title` 和 `--body`。
`create_low_risk_pr_from_proposal` 默认使用 `gh_low_risk_pr_create`，并传入
`--repo`、`--proposal-id`、`--issue`、`--title`、`--body`。该本地 wrapper 必须自己校验低风险文件范围、运行验证、创建标题以 `[auto-review]` 开头的 PR，并且不能自动 merge 或发版。
...
| `LARK_CRON_TIMEZONE` | 系统时区 | 新建 cronjob 的默认 IANA 时区（如 `Asia/Shanghai`、`UTC`）。每个 job 会在 `meta.timezone` 固化自己的时区；之后修改该环境变量不会静默改变已有任务的执行语义 |
...
| `LARK_AUDIT_LOG` | `~/.codex/channels/lark/audit.log` | 追加式文本行审计日志路径。每次敏感工具调用都会追加一行（尽力而为，写入失败不影响工具行为）（v0.11.0+）|
| `LARK_LOCAL_CLI_TOOLS_CONFIG` | `~/.codex/channels/lark/local-cli-tools.json` | `run_local_cli_tool` 的本地 CLI 白名单配置路径（v1.1.0+）|
| `LARK_GITHUB_ISSUE_TIMEOUT_MS` | `30000` | 内置 GitHub HTTP issue 创建的超时时间。 |
| `LARK_GITHUB_API_BASE_URL` | `https://api.github.com` | 内置 GitHub issue 创建使用的 GitHub API base URL。 |
| `LARK_GITHUB_TOKEN` | （空） | 内置 GitHub HTTP issue 创建使用的 token；未设置时会继续使用 `GH_TOKEN` 或 `GITHUB_TOKEN`。 |
...
| `create_default_review_jobs` | `(target_repo, target_chat_id, timezone?, chat_id, thread_id?)` | 创建暂停状态的自复盘和低风险自修复 cronjob 预置任务。默认不开启，只有显式 `update_job status=active` 后才会运行 |
...
| `create_github_issue` | `(title, body, target_repo, tool?, chat_id, thread_id?)` | 用户明确授权后直接创建 GitHub issue；默认走内置 GitHub HTTP path，可选 `tool` 只能指向已配置的本地 CLI override |
| `create_issue_proposal` | `(title, body, evidence?, impact?, priority?, automation_level?, target_repo, target_chat_id?, chat_id, thread_id?)` | 创建本地待审批 GitHub issue proposal，不会直接提 issue |
| `list_issue_proposals` | `(status?, chat_id, thread_id?)` | 列出当前可见的 issue proposal。私聊展示 caller 创建的；群里展示投递到当前群的 |
| `reject_issue_proposal` | `(id, reason?, chat_id, thread_id?)` | 拒绝一个 pending proposal。仅创建者或配置的 owner 可操作 |
| `create_issue_from_proposal` | `(id, tool?, chat_id, thread_id?)` | 用户明确授权后，通过内置 proposal filing path 提 GitHub issue；可选 `tool` 只能指向已配置的本地 CLI override |
| `create_low_risk_pr_from_proposal` | `(id, tool?, chat_id, thread_id?)` | 已创建 GitHub issue 的低风险 proposal 可通过 allowlisted 本地 CLI 工具（默认 `gh_low_risk_pr_create`）开 PR；插件不会自动 merge 或发版 |
