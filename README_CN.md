...
[![version](https://img.shields.io/badge/version-1.9.4-informational)](CHANGELOG.md)
...
只有用户明确授权后，`create_issue_from_proposal` 才会先尝试标准 GitHub CLI：
`gh issue create --repo <owner/name> --title <title> --body <body>`，
失败后在配置 token 时降级到 GitHub HTTP API。高级用法仍可显式传入 `tool`
使用 `gh_issue_create` 这类本地 CLI override。已经创建 GitHub issue、且标记为
...
`create_issue_from_proposal` 默认使用内置 GitHub issue 创建器：
`gh issue create --repo <owner/name> --title <proposal title>
--body <proposal body>`；当配置了 `LARK_GITHUB_TOKEN`、`GH_TOKEN` 或
`GITHUB_TOKEN` 时，`gh` 失败会继续走 HTTP API fallback。显式传入 `tool`
时仍可使用高级本地 CLI override；此时需要在对应工具的 `paramAllowlist`
中包含 `--repo`、`--title` 和 `--body`。
...
| `LARK_GITHUB_ISSUE_GH_COMMAND` | `gh` | `create_issue_from_proposal` 在 HTTP fallback 前优先调用的 `gh` 可执行文件。 |
| `LARK_GITHUB_ISSUE_TIMEOUT_MS` | `30000` | `gh issue create` 和 GitHub HTTP issue 创建的超时时间。 |
| `LARK_GITHUB_ISSUE_MAX_OUTPUT_BYTES` | `65536` | `gh issue create` 捕获 stdout/stderr 的最大字节数。 |
| `LARK_GITHUB_API_BASE_URL` | `https://api.github.com` | HTTP fallback 使用的 GitHub API base URL。 |
| `LARK_GITHUB_TOKEN` | （空） | 可选 HTTP fallback token；未设置时会继续使用 `GH_TOKEN` 或 `GITHUB_TOKEN`。 |
...
| `create_issue_from_proposal` | `(id, tool?, chat_id, thread_id?)` | 用户明确授权后，通过内置 `gh issue create` 和 HTTP fallback 提 GitHub issue；可选 `tool` 保留高级本地 CLI override |
