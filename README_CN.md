...
配置（默认 `gh_issue_create`）包装 `gh issue create` 来真正创建 GitHub issue。没有本地工具配置时，
proposal 会留在本地，Codex 不能声称 issue 已创建。
