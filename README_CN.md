...
/task retry ID     用新 Job ID 克隆失败/已取消任务
...
| `LARK_CONTINUATION_MAX_STEPS` | `24` | 单个 Job 最大持久化步骤数（`1`-`100`） |
| `LARK_CONTINUATION_MAX_RETRIES` | `3` | 单步可重试执行失败次数（`0`-`10`） |
| `LARK_CONTINUATION_MAX_AGE_HOURS` | `24` | Job 最大存活时间（`1`-`168` 小时） |
...
既要在 Job 中声明，也要继续通过当前 `local-cli-tools.json` 策略；网络始终关闭，且
不会继承前台 approval 或用户配置。已有 v1/v2 SQLite Job 会保守迁移。
...
`approval_policy=never`、关闭 sandbox 网络、忽略用户 Codex config，不能发送消息、
创建嵌套任务或执行源码发布；插件不提供 continuation MCP tool。
