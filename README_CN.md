...
[![version](https://img.shields.io/badge/version-2.8.0-informational)](CHANGELOG.md)
...
v2.0.0 采用一次性切换，依赖 Node.js 24.15.0 及以上版本提供的内置
`node:sqlite`。项目不保留 legacy continuation 实现或灰度分支；升级插件前需要先
升级 Node。需要回滚时应重新安装 v1.21.4，而不是开启隐藏兼容代码。
...
continuation 初始化失败只会关闭该能力，普通 chat 和 cronjob 仍可运行。
