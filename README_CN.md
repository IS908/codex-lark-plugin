...
[![version](https://img.shields.io/badge/version-1.9.3-informational)](CHANGELOG.md)
...
只有用户明确授权后，`create_issue_from_proposal` 才会通过可信的 `run_local_cli_tool`
配置（默认 `gh_issue_create`）包装 `gh issue create` 来真正创建 GitHub issue。已经创建 GitHub issue、且标记为
`low-risk-auto-pr-eligible` 的 proposal，可继续通过 `create_low_risk_pr_from_proposal`
调用 `gh_low_risk_pr_create` 这类受控 wrapper 开 PR。没有本地工具配置时，
proposal 会留在本地，Codex 不能声称 issue 或 PR 已创建。
...
`create_issue_from_proposal` 默认使用 `gh_issue_create`，并传入
`--repo=<owner/name>`、`--title=<proposal title>`、`--body=<proposal body>`。
启用该流程时需要在 `paramAllowlist` 中包含 `--repo`。
...
| `create_issue_from_proposal` | `(id, tool?, chat_id, thread_id?)` | 用户明确授权后，通过 allowlisted 本地 CLI 工具（默认 `gh_issue_create`）提 GitHub issue，并回写 issue URL |
