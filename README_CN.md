...
[![version](https://img.shields.io/badge/version-2.13.2-informational)](CHANGELOG.md)
...
- **原生出站 @mention**：普通文本回复、自动生成的 Schema 2.0 卡片和 Cron prompt 报告中的 `@显示名`，只有在 `im:chat:readonly` 将该名称唯一解析为当前会话成员时才会转成真正的飞书 @。未知/重名、邮箱、行内/围栏代码以及成员查询失败都会保留原文；raw 卡片和消息 payload 不做改写。
- 编辑已发送的消息
...
- **两种任务类型**：`message` 不运行 Codex，直接提交固定内容；`prompt` 运行只负责生成结果的 `codex exec`。两者都会先把输出事务化写入 durable outbox，再投递到飞书。
...
- 引用机器人生成的 cronjob 报告并要求“重跑”时，exec bridge 会派生可信的原 job id，通过 `run_job` 立即复用持久化任务定义；仅任务创建者可触发，手动重跑不会改变未来调度时间或暂停状态
- 崩溃恢复只补入最近一次错过的 occurrence。执行开始前的中断可安全恢复；Codex 已启动后的 prompt 中断会标记为 outcome-unknown 并阻塞，不会盲目重跑。
- 任务定义和 schedule cursor 仍以 JSON 文件存储在 `~/.codex/channels/lark/jobs/`；Run、Attempt、lease 和 delivery outbox 状态存储在 `~/.codex/channels/lark/runtime/continuations/jobs.sqlite`，JSON runtime 字段仅作为兼容投影。
- 执行重试与投递重试相互独立。飞书投递失败不会重新生成报告，发送结果不明确时不会盲目重发，永久目标错误只会自动暂停匹配同一 Job 定义 revision 的任务。
- `LARK_CONTINUATION_ENABLED=false` 只关闭 Async Task，不关闭共享 worker 上的 Cron workload。共享 SQLite 初始化失败时 Cron 会 fail-closed，不会回退到已删除的 Scheduler 执行器。

### 持久化后台续跑（Continuation）

- 当前台私聊、群聊或文档评论明确要求后台执行、持续监控、等待结果或完成后通知时，Codex 可以提交一个结构化 `create_continuation_job` exec action；普通任务和仅仅较重的任务默认看不到该 action，用户会立即收到包含持久化 Job ID 的确认消息
...
- `/task list|status|cancel|retry|retain|delete` 绕过 Codex 直接执行；list 支持 `--status` 过滤，creator 和 owner 可切换 `retain` 以跳过自动清理。retry 会创建新 Job ID，可重试 partial/blocked/failed/cancelled 任务
...
- 单实例锁，防止重复启动。混合版本升级期间，如果要修改 `LARK_APP_ID`，必须先停止所有旧版
  plugin 进程；新 runtime 可以保留当前 app 及已运行的同一系统用户 legacy 锁名，但无法预知
  之后才以另一个 app ID 启动的旧版进程。
- 发送者/群聊白名单过滤（两个列表同时配置时为 OR 关系）
- 定时任务 durable 崩溃恢复（只补最近一次错过的 occurrence，并分别恢复执行与投递）
...
2. 启用「机器人」能力
3. 添加以下权限：`im:message.p2p_msg:readonly`、`im:message.group_at_msg:readonly`、`im:message:send_as_bot`、`im:resource`、`im:message.reactions:write_only`、`im:message.reactions:read`、`docs:document.comment:read`、`docs:document.comment:create`、`drive:drive.metadata:readonly`；推荐增加 `im:chat:readonly`，用于将出站 `@显示名` 唯一解析为原生 mention
...
`/access` 和 `/access list` 只返回当前调用者/当前群聊的状态，例如
`User access: allowed`、`Chat access: allowed`、`No-mention mode: enabled`。
普通状态查询和 add/remove 结果不会返回完整配置 ID 列表。owner 需要查看完整
runtime access 配置时，使用上面的显式 admin list 命令。
...
/task list [--status pending,failed]
                   列出持久化后台任务，可按状态过滤
/task status ID    查看有权限任务的执行和投递状态
/task cancel ID    取消排队中或运行中的任务
/task retry ID     用新 Job ID 克隆未完整交付的终态任务
/task retain ID on|off
                   开启或关闭自动清理豁免
/task delete ID    脱敏删除终态任务
...
多次执行。本地 trace 行只展示紧凑 run id（`TRACE_RUN_ID_DISPLAY_LENGTH=16`，
去掉分隔符）以保持日志可读，但查询仍可传完整内部 `run_id`。启用 trace 后，
exec action bridge 会暴露有界 `get_run_trace` 查询：只能查询当前/引用消息、有权限
的 cronjob，或 creator/owner 可见的 continuation Job，并只返回脱敏结构化摘要，
不暴露原始 `trace.log` 内容。
debug、audit 和 trace 日志时间都使用 `LARK_CRON_TIMEZONE`
...
exec delivery 也内置父进程 action bridge，用于处理子 `codex exec` 进程不能安全调用当前 MCP server 的内置动作：`save_memory`、`create_job`、`list_jobs`、`run_job`、`update_job`、`disable_job`、`delete_job`、`upsert_job`、`run_local_cli_tool`、`manage_access_control`、`send_message` 和 `recall_message`。父进程会为每个 `codex exec` turn 创建 owner-only 的 action JSONL 侧通道，只把文件路径和 per-turn token 交给子进程；子进程把结构化 action request 写入该侧通道，stdout 始终只作为用户可见回复文本。父进程校验 token/schema，拒绝子进程提供的身份字段，使用当前飞书事件派生调用者身份，在本地执行通过校验的动作；格式错误的 action request 会被标记为失败，而不是递归加载 Lark MCP server。`run_job` 仅允许任务创建者调用，并同步复用原 prompt、model、目标 chat 和 scheduler/report 路径。`send_message` 可用插件运行时身份发送本地 image/file、当前入站消息已下载的第一张图片，或 quoted/replied message 的第一张图片；也支持有序 `kind=rich` 文本+图片 parts，优先发送一条飞书富文本 post，失败时按顺序拆分消息并保持 thread context。音视频和交互卡片仍是后续独立设计切片。

外部项目管理系统写操作不再内置在 Lark plugin 核心里。创建 GitHub/GitLab issue、Jira ticket、Linear issue、PR，或周期复盘 proposal，应由用户自定义 skill、custom MCP、Codex runtime 已有工具，或 allowlisted `run_local_cli_tool` 承载。插件只提供 Lark channel、身份、cronjob、审计和通用本地工具边界；具体 provider 的策略应放在插件之外。

因为 exec delivery 是单轮流程，插件也会拦截容易误导的“回复后继续办事”承诺。最终回答不能声称 Codex 会在可见飞书回复发出后继续创建、提交、回贴链接或稍后处理，除非本轮成功创建了明确的 continuation 或 cronjob；其他 structured action 以及 `[LARK_DEFER]` / `[LARK_NO_REPLY]` 都不代表后台工作已建立。guard 会忽略 fenced code、引用文本、设计讨论和否定说明；真正未兑现的后续承诺会被替换为安全说明。
...
`run_local_cli_tool` 是可选的 sandbox host-tool bridge，用于可信的 host-local CLI 或
skill-backed workflow，例如 `lark-cli`。只有当 `LARK_CODEX_EXEC_SANDBOX` 为
`read-only` 或 `workspace-write`，且至少配置了一个 allowlisted 工具时，才会在
`codex exec` prompt 里暴露。它是附加能力，不是外部系统操作的唯一通道：Codex 仍可使用当前
runtime 已有的 skill、connector、MCP tool、runtime tool，或在可用时使用普通 CLI。缺少
bridge 配置不应直接导致 Codex 停在草稿。

这个 bridge 不会执行 shell 字符串，也不会改变普通 `codex exec` 的沙箱设置。前台调用从
`IdentitySession` 解析真实调用者；continuation 调用使用认证来源事件持久化的 creator 身份。
两条路径都按同一份工具配置授权，应用一种参数过滤模式，使用
...
执行中的调用视为外部结果不确定，任务会进入 blocked，而不会自动重放。
...
### 持久化 Continuation Runtime

durable runtime 依赖 Node.js 24.15.0 及以上版本提供的内置 `node:sqlite`。
v2.9.0 将 Async Task 和 Cron 的 Run、Attempt、lease、outbox 机制一次性切换到
同一个通用 kernel，不保留 legacy Cron executor、双写、灰度开关或 fallback。
Cron 定义仍使用 JSON，只有运行与投递历史进入 SQLite。

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `LARK_CONTINUATION_ENABLED` | `true` | 启用持久化后台 continuation 的创建和执行 |
| `LARK_CONTINUATION_MAX_CONCURRENCY` | `1` | 并发 continuation 数（`1`-`4`） |
| `LARK_CONTINUATION_MAX_ATTEMPTS` | `5` | 单个 Job 最大执行 attempt 数（`1`-`20`） |
| `LARK_CONTINUATION_MAX_RETRIES` | `3` | attempt 预算内的可重试执行失败次数（`0`-`10`） |
| `LARK_CONTINUATION_MAX_TOTAL_MINUTES` | `30` | Job 最大存活时间（`5`-`1440` 分钟） |
| `LARK_CONTINUATION_RETENTION_DAYS` | `30` | 终态任务正文和托管 artifact 脱敏前的保留天数 |
| `LARK_CONTINUATION_WORKING_ROOT` | `LARK_CODEX_EXEC_CWD` | continuation 工作目录的绝对授权根目录 |

倒数第二个 attempt 会收到收敛警告。最后一个 attempt 必须返回 `completed`、
`partial`、`blocked` 或 `failed`；若模型仍返回 `continue`，父进程会根据 checkpoint
确定性转换为 `partial`。

自动 retention 从 `completed_at` 开始计时，只清理 terminal delivery 已确认
`delivered` 且 `retain=off` 的终态 Job。清理会删除 attempt/tool-call 明细、progress
事件和托管 artifact，同时保留紧凑 terminal tombstone 与逐项审计结果。非终态、
已 retain、尚未投递或本轮清理失败的 Job 会留待后续扫描。

continuation action 的 `working_directory` 必须相对该 root。例如配置
`LARK_CONTINUATION_WORKING_ROOT=/Users/you/workspace` 后，使用
...
fail closed 为 blocked。未来 coordinator 必须把一次性审批绑定到 operation digest、
请求者/审批者身份、过期时间、审批决定和单次消费记录。

Job、attempt、checkpoint、lease 和终态 outbox 以事务方式存储在
`~/.codex/channels/lark/runtime/continuations/jobs.sqlite`，托管 artifact 位于同级
`artifacts/`。每个 Job 使用独立 Codex execution session；前台 session 只作为来源，
resume 失效时会安全替换，不会改动前台 chat session。后台 runner 固定
`approval_policy=never` 并忽略用户 Codex config；默认 profile 禁网且禁止源码发布，trusted profile
按上文显式权限快照启用全盘读取、网络和外部操作。后台不能调用前台 action bridge、发送 Lark
消息或创建嵌套任务；插件不提供 continuation MCP tool。

`/task` 直接由父进程处理：creator 只能管理自己的任务，`LARK_OWNER_OPEN_ID` 可管理
全部任务。IM 终态消息在 Feishu UUID 一小时去重窗口内安全重试；文档评论在模糊结果后
按 Job marker 有界回读。无法确认时状态变为 `delivery_unknown`，不会盲目重发或重跑。
`LARK_CONTINUATION_ENABLED=false` 只从共享 worker 移除 Async Task，Cron 仍可运行。
SQLite 初始化失败会 fail-closed 地关闭 Async Task 与 Cron 执行；普通前台 chat 仍可用，
且不会启动 legacy Cron fallback。
...
| `run_local_cli_tool` | `(tool, args?, chat_id, thread_id?)` | 可选 sandbox host-tool bridge，用于在插件宿主机运行已配置白名单的本地 CLI 能力；它是附加能力，不是外部系统操作的唯一通道。调用者身份由 `chat_id` / `thread_id` 服务端派生；参数和环境由 `local-cli-tools.json` 过滤（v1.1.0+）|
