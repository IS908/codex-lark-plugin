...
- **追加式审计日志（v0.11.0+）**：`~/.codex/channels/lark/audit.log` 以 JSONL 记录每次敏感工具调用（`at` / `kind` / 工具名 / 调用者 / 结果 / 脱敏参数），运营者可事后回溯查看本机上发生了什么
...
| `LARK_CODEX_EXEC_TRACE_LOG` | `~/.codex/channels/lark/trace.log` | 本地 codex exec tool trace JSONL 日志路径 |

当 `LARK_CODEX_EXEC_TOOL_TRACE=true` 时，父进程会扫描 `codex exec --json`
stdout 中的工具执行事件，并把脱敏 JSONL 记录追加到
`LARK_CODEX_EXEC_TRACE_LOG`。这只用于本地排障：飞书仍只接收最终回答或受限
progress 消息。`compact` 记录工具名、参数摘要、状态、耗时和错误摘要；
`full` 记录脱敏/截断后的事件结构。audit 和 trace 日志共享
[本地诊断日志 JSONL 格式](docs/local-diagnostic-logs.md)。
...
| `LARK_AUDIT_LOG` | `~/.codex/channels/lark/audit.log` | 追加式 JSONL 审计日志路径。每次敏感工具调用都会追加一行（尽力而为，写入失败不影响工具行为）（v0.11.0+）|
