...
- 后台 runner 固定 `approval_policy=never`、关闭 sandbox 网络、忽略用户 Codex config，不能发送消息、创建嵌套任务或执行源码发布；插件不提供 continuation MCP tool
...
这个 bridge 不会执行 shell 字符串，也不会改变普通 `codex exec` 的沙箱设置。每次调用都会从
