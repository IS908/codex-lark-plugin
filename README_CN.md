...
`plugin-low-risk-auto-fix`。两者创建后都是 `weekly on fri at 18:00` 且
`status=paused`，不会自动运行；用户必须显式 resume 后才会开始自复盘或低风险自修复。自复盘任务只
创建 proposal 并汇报到飞书；低风险自修复任务只允许 docs/tests/metadata 等低风险工作，不能自动
merge PR，也不能自动发版。
