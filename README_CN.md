...
bridge 不再提供 GitHub issue 创建这类领域专用 built-in。若确实需要创建 issue，应通过
`run_local_cli_tool` 配置一个可信的本地工具包装 `gh issue create`，并在该工具配置里显式设置
allowlist、固定参数、超时和输出上限；如果没有这样的本地工具配置，Codex 应给出 issue 草稿，
而不是声称已经创建。
