...
| Cron prompt execution | Route through the scheduler prompt runner and the same `codex exec` delivery path as chat messages. | Prevents prompt cronjobs from timing out after an out-of-band host acknowledgement without starting a real Codex exec turn. |
