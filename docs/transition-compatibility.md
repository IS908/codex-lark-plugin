...
| Persistent continuation runtime | Added as a direct v2.0.0 cutover using built-in `node:sqlite`; there is no legacy in-memory path, notification fallback, dual-write, or rollout selector. | Keep one SQLite/lease/outbox implementation. `LARK_CONTINUATION_ENABLED` disables the capability; it is not a compatibility mode. Mixed-version locking covers the current app ID and same-user legacy locks already present at startup; operators must stop every old process before changing `LARK_APP_ID`. | Continuation repository, worker, command, delivery, runtime, cross-process restart, and both same-app legacy/new lock start orders must pass on Node 24.15.0. | Rollback is package downgrade to v1.21.4 after stopping the v2 process. The v2 continuation database may be retained for a later re-upgrade. | Do not add transition branches; evolve schema through explicit `user_version` migrations. |
