...
src/scheduler.ts    – JobScheduler: periodic scan (60s), idempotent durable-run admission, schedule cursor projection
src/queue.ts        – Per-thread sequential message queue
src/durable-run/    – Shared durable-run kernel: SQLite repository, worker, workload registry, leases, attempts, and outbox
src/continuation/   – Async Task workload: Codex runner, commands, checkpoints, and terminal delivery
src/cron/           – Cron workloads: prompt/message runners, Feishu delivery, and JSON runtime projection
...
**CronJob flow (v2.9.0+):** `JobScheduler.tick()` every 60s → read JSON Job definitions → idempotently admit due or manual Runs into the shared SQLite durable-run kernel → a leased worker executes the registered Cron prompt/message workload → a transactional outbox delivers the result through Feishu → exact revision/Run fences project terminal state back to the JSON schedule cursor. Scheduler never executes or delivers Cron payloads, and there is no legacy fallback or dual write. JSON remains authoritative for Job definitions and schedule cursors; SQLite is authoritative for Run, Attempt, lease, and outbox history.

**Continuation flow (v2.9.0+):** a foreground `codex exec` turn with explicit current-user async intent may commit one parent-owned `create_continuation_job` action → the shared SQLite durable-run repository atomically persists the Async Task Job and route → a bounded workload worker claims it with a lease and runs structured Codex steps in a dedicated session → checkpoints and Attempts are persisted transactionally → terminal state creates an outbox row → the delivery adapter sends progress and one final IM or document-comment result. Ordinary/heavy turns do not receive this action or permission; routing classification uses unenriched current-user text, never quoted content or memory. `/task list|status|cancel|retry|retain|delete` bypass Codex and authorize against the authenticated creator or `LARK_OWNER_OPEN_ID`. `LARK_CONTINUATION_ENABLED=false` disables only Async Task admission; Cron remains available when shared persistence initializes. The runtime requires Node.js 24.15.0 or newer for built-in `node:sqlite`.
...
- **Shared durable-run runtime**: Async Task and Cron workloads use SQLite WAL, process-safe leases, workload-specific concurrency quotas and retry policy, startup recovery, and a transactional terminal outbox. Async Task restrictions remain workload-owned; Cron execution and delivery are never delegated back to Scheduler.
...
Root `src/` is the only application source tree. `npm run build:plugin` creates
the checked-in, self-contained `plugins/lark/runtime/{index,stop,doctor}.js`
entrypoints consumed from the marketplace clone and versioned Codex runtime
cache. Never edit generated runtime bundles directly or add a mirrored source
tree under `plugins/lark/`.
