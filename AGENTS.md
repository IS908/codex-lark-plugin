...
**CronJob flow:** `JobScheduler.tick()` every 60s → read all job files → for each active job where `next_run_at <= now` → execute (message: direct Feishu API / prompt: in `exec` mode, run through the same `codex exec` delivery path as chat messages under a unique `thread_id` + bind session identity to `job.created_by`) → update `runtime` in job file. On startup, `recoverMissedJobs()` runs the same check once for crash recovery. The exec-only `run_job` action resolves creator ownership and invokes this same scheduler path immediately; future schedules and paused state are preserved, and overlapping runs are rejected.

**Continuation flow (v2.0.0+):** a foreground `codex exec` turn with explicit current-user async intent may commit one parent-owned `create_continuation_job` action → the SQLite repository atomically persists the Job and route → a bounded worker claims it with a lease and runs structured Codex steps in a dedicated session → checkpoints and attempts are persisted transactionally → terminal state creates an outbox row → the delivery adapter sends progress and one final IM or document-comment result. Ordinary/heavy turns do not receive this action or permission; routing classification uses unenriched current-user text, never quoted content or memory. `/task list|status|cancel|retry|retain|delete` bypass Codex and authorize against the authenticated creator or `LARK_OWNER_OPEN_ID`. The runtime requires Node.js 24.15.0 or newer for built-in `node:sqlite`.
