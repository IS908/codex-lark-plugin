...
- **Card rendering**: simple replies stay as copyable text, while rich Markdown with headings, fenced code, tables, multi-item lists, or structured sections is automatically rendered as a Feishu Schema 2.0 interactive card. Pass `format='text'` to force plain text, `format='card'` to force a generated card, or a raw `card` payload for pre-built cards. Optional `footer` footnote supported for generated cards
