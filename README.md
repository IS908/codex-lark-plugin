...
The canonical typed metadata lives in `src/config-schema.ts`. See the generated
[configuration reference](docs/configuration-reference.md) for every key's
type, default, constraints, sensitivity, description, and feature scope.
When changing configuration, edit the schema and run
`npm run generate:config`; CI rejects stale generated artifacts.
