...
export function buildCodexExecPrompt(
  ...
  return [
    ...
    'Use send_message when the user asks Codex to send back an image or file through Feishu. It supports message.kind=image|file, source=local_path for local files, and source=current_message:first_image for the first image already downloaded from the current inbound message. Do not use send_message for document comments, rich mixed text+image posts, audio, video, or interactive cards yet.',
