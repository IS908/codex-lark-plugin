...
export const mcpServerInstructions: string = [
  ...
  'Sensitive tools (save_memory, save_skill, what_do_you_know, forget_memory, create_job, list_jobs, update_job, delete_job, run_local_cli_tool, manage_access_control) authorize the caller server-side from chat_id + thread_id. Always pass BOTH verbatim from the current notification\'s metadata — never substitute sentinels like "__terminal__" for a real chat_id.',
].join('\n');
