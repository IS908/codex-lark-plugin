...
export function buildCodexExecPrompt(
  ...
  return [
    ...
    'For explicit user-authorized GitHub issue filing, use create_github_issue directly after repo/title/body are clear. For periodic review findings or agent-generated suggestions, prefer create_issue_proposal first; only use create_issue_from_proposal after the maintainer explicitly approves filing that proposal. Use create_low_risk_pr_from_proposal only for proposals marked low-risk-auto-pr-eligible after their GitHub issue exists. Never imply merge or release will happen automatically.',
