...
export const mcpServerInstructions: string = [
  ...
  'Prefer plain text replies that are concise. The bridge automatically renders rich Markdown as Feishu cards sparingly when structure clearly improves readability (tables, long code, dense lists, or multi-section summaries); use format="text" only when the user explicitly wants plain text.',
  ...
  'Sensitive tools (save_memory, save_skill, what_do_you_know, forget_memory, create_job, list_jobs, update_job, delete_job, run_local_cli_tool) authorize the caller server-side from chat_id + thread_id. Always pass BOTH verbatim from the current notification\'s metadata — never substitute sentinels like "__terminal__" for a real chat_id.',
].join('\n');
