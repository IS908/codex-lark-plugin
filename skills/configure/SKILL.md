...
- `LARK_CODEX_EXEC_TOOL_TRACE` — write local codex exec tool-call trace JSONL records to trace.log; never renders tool traces into Feishu replies (default: false)
...
- `LARK_CODEX_EXEC_TRACE_LOG` — local codex exec tool trace JSONL log path (default: `~/.codex/channels/lark/trace.log`)
